...
class ReviewCreateView(MyCreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        qry_review_add = Review(
            name=data['name'].strip(), slug=slug.strip(), description=data['description'].strip(),
            more_details=data['more_details'].strip(), meta_title=data['meta_title'].strip(),
            status=data['status'].strip(), publish_date=data['publish_date'], youtube_video=data['youtube_video'].strip(),
            create_by=request.user, featured_image=request.FILES.get('featured_image', None),
            model_name=data['model_name'].strip(), model_year=data['model_year'].strip(), trim=data['trim'].strip()
        )
        qry_review_add.save()
        ...
        if data['motor_nominal_output'] == "":
            ...
            data['motor_nominal_output'] = data['motor_nominal_output'].split()

        if data['battery_watt_hours'] == "":
            data['battery_watt_hours'] = None
        else:
            data['battery_watt_hours'] = data['battery_watt_hours'].strip()
        ...
        qry_review_general_add = ReviewGeneral(
            msrp=data['msrp'].strip(), bike_class=add_bike_class, suspension=data['suspension'],
            frame_type=add_frame_type, gears=data['gears'], demo_gear=data['gears'],
            motor_type=data['motor_type'],  wheel_size=add_wheel_size, brake_type=add_brake_type,
            motor_nominal_output=data['motor_nominal_output'],
            demo_motor_nominal_output=data['motor_nominal_output'],
            battery_watt_hours=data['battery_watt_hours'], demo_battery_watt_hours=data['battery_watt_hours'],
            demo_weight=data['weight'], weight=data['weight'], review=qry_review_add
        )
        ...
        if data['frameset_weight'] == "":
            ...
            data['frameset_weight'] = data['frameset_weight'].strip()

        if data['suspension_travel'] == "":
            data['suspension_travel'] = None
        else:
            data['suspension_travel'] = data['suspension_travel'].strip()

        qry_review_frameset_add = ReviewFrameset(
            frameset_frame_type=add_frame_type, demo_frameset_weight=data['frameset_weight'],
            frameset_weight=data['frameset_weight'], load_capacity=data['load_capacity'],
            frameset_suspension=data['frameset_suspension'], demo_suspension_travel=data['suspension_travel'],
            suspension_travel=data['suspension_travel'], fork=data['fork'].strip(),
            frameset_wheel_size=add_wheel_size, rear_shock=data['rear_shock'].strip(),
            front_wheel=data['front_wheel'].strip(), rear_wheel=data['rear_wheel'].strip(), front_hub=data['front_hub'].strip(),
            rear_hub=data['rear_hub'].strip(), tires=data['tires'].strip(), review=qry_review_add
        )
        ...
        if data['drivetrain_gears'] == "":
            ...
            data['suspension_travel'] = data['suspension_travel'].split()

        qry_review_drivetrain_add = ReviewDrivetrain(
            drivetrain_gears=data['drivetrain_gears'], demo_drivetrain_gears=data['drivetrain_gears'],
            shift_levers=data['shift_levers'].strip(), front_derailleur=data['front_derailleur'].strip(),
            crankset=data['crankset'].strip(), rear_derailleur=data['rear_derailleur'].strip(),
            electronic_shifting=data['electronic_shifting'].strip(), internally_geared_hub=data['internally_geared_hub'].strip(),
            continually_variable_transmission=data['continually_variable_transmission'].strip(), cassette=data['cassette'].strip(),
            chainring=data['chainring'].strip(), belt_drive=data['belt_drive'].strip(), review=qry_review_add
        )
        ...
        if data['seatpost_diameter'] == "":
            ...
            data['seatpost_diameter'] = data['seatpost_diameter'].split()

        qry_review_components_add = ReviewComponents(
            headset=data['headset'].strip(), stem=data['stem'].strip(), handlebar=data['handlebar'].strip(), grips=data['grips'].strip(),
            seatpost=data['seatpost'].strip(), seatpost_diameter=data['seatpost_diameter'],
            demo_seatpost_diameter=data['seatpost_diameter'], saddle=data['saddle'].strip(),
            pedals=data['pedals'].strip(), components_brake_type=add_brake_type, front_brake=data['front_brake'].strip(),
            rear_brake=data['rear_brake'].strip(), review=qry_review_add
        )
        ...
        if data['systems_motor_nominal_output'] == "":
            ...
            data['systems_motor_nominal_output'] = data['systems_motor_nominal_output'].split()

        if data['systems_battery_watt_hours'] == "":
            data['systems_battery_watt_hours'] = None
        else:
            data['systems_battery_watt_hours'] = data['systems_battery_watt_hours'].split()

        qry_review_ebike_systems_add = ReviewEbikeSystems(
            systems_bike_class=add_bike_class, systems_motor_type=data['systems_motor_type'],
            motor=data['motor'].strip(), additional_motors=data['additional_motors'].strip(),
            systems_motor_nominal_output=data['systems_motor_nominal_output'],
            demo_systems_motor_nominal_output=data['systems_motor_nominal_output'], display=data['display'].strip(),
            smart_bike=data['smart_bike'].strip(), theft_gps=data['theft_gps'].strip(),
            demo_systems_battery_watt_hours=data['systems_battery_watt_hours'],
            systems_battery_watt_hours=data['systems_battery_watt_hours'], battery=data['battery'].strip(),
            additional_battery=data['additional_battery'].strip(),
            charger=data['charger'].strip(), review=qry_review_add
        )
        ...
        qry_review_accessories_add = ReviewAccessories(
            lights=data['lights'].strip(), fenders=data['fenders'].strip(),
            front_rack=data['front_rack'].strip(), rear_rack=data['rear_rack'].strip(), review=qry_review_add
        )
...
class ReviewUpdateView(MyUpdateView):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        Review.objects.filter(id=pk).update(
            name=data['name'].strip(), description=data['description'].strip(), more_details=data['more_details'].strip(),
            meta_title=data['meta_title'].strip(), status=data['status'].strip(), publish_date=data['publish_date'],
            youtube_video=data['youtube_video'].strip(), create_by=request.user,
            model_name=data['model_name'].strip(), model_year=data['model_year'].strip(), trim=data['trim'].strip(), slug=slug.strip()
        )
        ...
        if data['battery_watt_hours'] == "":
            ...
            data['battery_watt_hours'] = data['battery_watt_hours'].split()
        ...
        ReviewGeneral.objects.filter(review=pk).update(
            msrp=data['msrp'].strip(), bike_class=add_bike_class, frame_type=add_frame_type, wheel_size=add_wheel_size,
            brake_type=add_brake_type, suspension=data['suspension'], gears=data['gears'],
            demo_gear=data['gears'], motor_type=data['motor_type'],
            motor_nominal_output=data['motor_nominal_output'], demo_motor_nominal_output=data['motor_nominal_output'],
            battery_watt_hours=data['battery_watt_hours'], demo_battery_watt_hours=data['battery_watt_hours'],
            weight=data['weight'], demo_weight=data['weight']
        )
        ...
        if data['frameset_weight'] == "" or data['frameset_weight'] == 'None':
            ...
            data['frameset_weight'] = data['frameset_weight'].split()
        ...
        ReviewFrameset.objects.filter(review=pk).update(
            ...
            frameset_suspension=data['frameset_suspension'], demo_suspension_travel=data['suspension_travel'],
            suspension_travel=data['suspension_travel'], fork=data['fork'].strip(), rear_shock=data['rear_shock'].strip(),
            frameset_wheel_size=add_wheel_size, front_wheel=data['front_wheel'].strip(), rear_wheel=data['rear_wheel'].strip(),
            front_hub=data['front_hub'].strip(), rear_hub=data['rear_hub'].strip(), tires=data['tires'].strip()
        )
        ...
        ReviewDrivetrain.objects.filter(review=pk).update(
            drivetrain_gears=data['drivetrain_gears'].strip(), demo_drivetrain_gears=data['drivetrain_gears'].strip(),
            shift_levers=data['shift_levers'].strip(), front_derailleur=data['front_derailleur'].strip(),
            crankset=data['crankset'].strip(), rear_derailleur=data['rear_derailleur'].strip(),
            electronic_shifting=data['electronic_shifting'].strip(), internally_geared_hub=data['internally_geared_hub'].strip(),
            continually_variable_transmission=data['continually_variable_transmission'].strip(), cassette=data['cassette'].strip(),
            chainring=data['chainring'].strip(), belt_drive=data['belt_drive'].strip()
        )
        ...
        ReviewComponents.objects.filter(review=pk).update(
            headset=data['headset'].strip(), stem=data['stem'].strip(), handlebar=data['handlebar'].strip(), grips=data['grips'].strip(),
            seatpost=data['seatpost'].strip(), seatpost_diameter=data['seatpost_diameter'], demo_seatpost_diameter=data['seatpost_diameter'],
            saddle=data['saddle'].strip(), pedals=data['pedals'].strip(), front_brake=data['front_brake'].strip(), rear_brake=data['rear_brake'].strip(),
        ...
        ReviewEbikeSystems.objects.filter(review=pk).update(
            systems_bike_class=add_bike_class, systems_motor_type=data['systems_motor_type'], motor=data['motor'].strip(),
            additional_motors=data['additional_motors'].strip(), systems_motor_nominal_output=data['systems_motor_nominal_output'],
            demo_systems_motor_nominal_output=data['systems_motor_nominal_output'],  display=data['display'].strip(),
            smart_bike=data['smart_bike'].strip(), theft_gps=data['theft_gps'].strip(),
            ...
            battery=data['battery'].strip(), additional_battery=data['additional_battery'].strip(), charger=data['charger'].strip()
        )
        ...
        ReviewAccessories.objects.filter(review=pk).update(
            lights=data['lights'].strip(), fenders=data['fenders'].strip(),
            front_rack=data['front_rack'].strip(), rear_rack=data['rear_rack'].strip()
        )
        if data['imageArray'] != '' and data['oldImageArray'] != '':
            new_review_images = eval(data['imageArray'])
            old_review_images = eval(data['oldImageArray'])
            print("new_review_images =======>>", new_review_images)
            print("old_review_images =======>>", old_review_images)
            ...
            for new_image in new_review_images:
                if new_image:
                    ...
                    if qry_gallery_image.exists():
                        if not ReviewGalley.objects.filter(id=image['image'], review=qry_review_update).exists():
