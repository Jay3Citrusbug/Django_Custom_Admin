...
$(document).ready(function(){
    ...
    for (let category in all_categories){
        if (all_categories[category].getAttribute("class").includes("active_category") === true){
            ...
            id_list.push(parseInt(all_categories[category].getAttribute("data-id")))
            ...
            id_list = id_list.filter(id_list => id_list !== parseInt(all_categories[category].getAttribute("data-id")))
            category_data_result(id_list)
        }
    }
...
function price_range(min_price, max_price) {
    var pre_entered_min_price = min_price
    var pre_entered_max_price = max_price
    $(".price-range").on('keyup change', function () {
        if ($(this).hasClass('min')) {
            ...
            if (parseInt(min_price) >= parseInt(max_price)) {
                ...
                $("#error_max_price").html('')
                $('#price-result').removeAttr("disabled");
        ...
        if (pre_entered_min_price == min_price && pre_entered_max_price == max_price){
            $('#price-result').prop("disabled", true);
        }
...
function price_range_ajax(min_price, max_price){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (window.location.href.includes("/search/") === true){
                advance_search_url = window.location.href + `&${this.url.split("?")[1]}`
            } else {
                advance_search_url = this.url.replace('/price-range/', '/search/')
            }
...
//                see_result.innerHTML = `See ${data['total_review'].length} Results`
            }

            if (data['total_review'].length == 0){
                $('#price-result').prop("disabled", true);
            }
        }
    })
...
$("#price-result").on("click", function(){
...
//            see_result.innerHTML = `See ${data['total_reviews'].length} Results`
            ...
            //     $('#more-filter-see-result').prop("disabled", true);
            // }
            var see_result_price_range = $("#price-result")[0];
//            see_result.innerHTML = `See ${data['total_reviews'].length} Results`
//            see_result_price_range.innerHTML = `See ${data['total_reviews'].length} Results`
//            see_result.innerHTML = `See ${data['total_reviews'].length} Results`
            // if(advance_search_url != '/search/'){
            //     $('#more-filter-see-result').removeAttr("disabled");
            // }
...
function more_filter_inputs(min_year, max_year, min_weight, max_weight, min_battery, max_battery, min_suspension_travel, max_suspension_travel,
    keywords, motor_type, bike_class,
    suspension, accessories_fenders,
    accessories_lights, accessories_racks) {
...
$(".page-filtered-data").on("click", function(){
    ...
    for (let tag in all_filtered_tags){
        if (all_filtered_tags[tag].getAttribute('data-tag') != this_tag || all_filtered_tags[tag].getAttribute('data-value') != this_value){
             if (all_filtered_tags[tag].getAttribute('data-tag') == "brands"){
            ...
            else if (all_filtered_tags[tag].getAttribute('data-tag') == "accessories_racks"){
                console.log(all_filtered_tags[tag].getAttribute('data-tag'), '==============');
                if (jQuery.inArray(all_filtered_tags[tag].getAttribute('data-value'), page_filtered_accessories ) == -1){
                    if (all_filtered_tags[tag].getAttribute('data-value') == "Fenders"){
                        ...
                        data["accessories_fenders"] = "Yes"
                    }
                    if (all_filtered_tags[tag].getAttribute('data-value') == "Lights"){
                        // page_filtered_accessories.push(data["accessories_lights"] = "Yes")
                        data["accessories_lights"] = "Yes"
                    }
                    if (all_filtered_tags[tag].getAttribute('data-value') == "Rack" || all_filtered_tags[tag].getAttribute('data-value') == "Rack"){
                        // page_filtered_accessories.push(data["accessories_racks"] = "Yes")
                        data["accessories_racks"] = "Yes"
                    }
                }
                ...
                if (jQuery.inArray( all_filtered_tags[tag].getAttribute('data-value'), page_filtered_keyword ) == -1){
                    ...
                    data[`${all_filtered_tags[tag].getAttribute('data-tag')}`] = page_filtered_keyword
